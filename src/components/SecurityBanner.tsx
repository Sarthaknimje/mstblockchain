    ...
    <section className="pb-16 md:pb-28">
      <ScrollReveal>
        ...
        >
          ...
          <motion.img src={mstLogo} alt="" className="absolute top-4 right-6 w-12 h-12 opacity-10 group-hover:opacity-20 transition-opacity" animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 6, repeat: Infinity }} />

          <div className="relative z-10">
            <span className="label-style text-primary mb-4 block">{t("security.label")}</span>
            <h3 className="text-3xl md:text-4xl font-black mb-4 uppercase tracking-tighter">
              {t("security.title")} <span className="primary-gradient-text">{t("security.title2")}</span>
            </h3>
            ...
            <div className="flex flex-wrap justify-center gap-3 mb-8">
              {[
                ...
                <span key={tag} className={`px-3 py-1 border ${color} text-[9px] font-black uppercase tracking-[0.15em]`}>{tag}</span>
              ))}
            </div>
            <motion.button className="mx-auto w-fit label-style border-b-2 border-primary pb-1 hover:text-primary transition-colors" whileHover={{ scale: 1.05 }}>
              {t("security.readDocs")}
            </motion.button>
          </div>
        </motion.div>
