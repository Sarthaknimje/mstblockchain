  ...
  return (
    <section className="py-16 md:py-28 section-border">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 md:mb-16 gap-4">
        <ScrollReveal>
          <span className="label-style text-primary mb-3 block">{t("blogs.label")}</span>
          <h2 className="text-4xl md:text-6xl font-black tracking-tighter uppercase">
            {t("blogs.title")} <span className="primary-gradient-text">{t("blogs.title2")}</span>
          </h2>
        </ScrollReveal>
        <div className="flex gap-3">
          {[ArrowLeft, ArrowRight].map((Icon, i) => (
            <motion.button key={i} className="w-10 h-10 border-2 border-foreground/20 flex items-center justify-center hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Icon size={18} />
            </motion.button>
          ))}
        </div>
        ...
        {blogs.map((blog, i) => (
          <ScrollReveal key={blog.title} delay={i * 0.1}>
            <motion.div className="group cursor-pointer" whileHover={{ y: -4 }}>
              ...
              <h3 className="text-lg md:text-xl font-black uppercase tracking-tighter mb-3 group-hover:text-primary transition-colors leading-tight">{blog.title}</h3>
              <p className="text-sm font-medium text-on-surface-variant mb-4 leading-relaxed">{blog.desc}</p>
              <span className="story-link label-style cursor-pointer">{t("blogs.readMore")}</span>
            </motion.div>
          </ScrollReveal>
        ))}
      </div>
